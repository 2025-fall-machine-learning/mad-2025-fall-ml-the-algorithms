...
def perform_logistic_regression(diabetes_predictors_df, diabetes_response_df,
    ...
    for random_state in range(0, 3):
        ...
        # ADDED TO HANDLE BALANCED DATASET
        if balance_counter == 1:
            ros = ios.RandomOverSampler(random_state=random_state)
            (diabetes_predictors_training_df,
            diabetes_response_training_df) = ros.fit_resample(
                diabetes_predictors_training_df, diabetes_response_training_df
            )
...
def main():
    diabetes_df = pd.read_csv('2_classification\pa_diabetes.csv')
    predict(diabetes_df)
