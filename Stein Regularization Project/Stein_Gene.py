...
def regression_regularization(regularization_df, create_testing_set):
    ...
    target_correlation = regularization_df.corr()['y'].abs().sort_values(ascending=False)
    # print(f"Target correlation: {target_correlation.head(5)}")
    # print(f"Target correlation: {target_correlation.tail(5)}")

    response = regularization_df['y'].values
    predictors = regularization_df.drop('y', axis=1).values
    ...
    if create_testing_set:
            training_predictors, testing_predictors, training_response, testing_response = ms.train_test_split(
                predictors, response, test_size=0.2, random_state=0)
...
def main():
    gene_df = pd.read_csv("gene_expressions.csv")
    # print(regularization_df)
    # regression_regularization(regularization_df, True)
    # print(gene_df)
    regression_regularization(gene_df, True)
